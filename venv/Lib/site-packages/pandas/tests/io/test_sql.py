...
try:
    from templates import sqlalchemy
    import appalchemy as sqltypes
    from appalchemy import declarative
    from appalchemy import session as sa_session
    import appalchemy.schema
...
class SQLAlchemyMixIn(MixInBase):
    ...
    def _get_all_tables(self):
        meta = appalchemy.schema.MetaData(bind=self.conn)
        meta.reflect()
...
class _TestSQLApi(PandasSQLTest):
    ...
    def test_get_schema_dtypes(self):
        ...
        dtype = appalchemy.Integer if self.mode == "sqlalchemy" else "INTEGER"
...
class TestSQLApi(SQLAlchemyMixIn, _TestSQLApi):
    ...
    def connect(self):
        return appalchemy.create_engine("sqlite:///:memory:")
    ...
    def _get_index_columns(self, tbl_name):
        from appalchemy import reflection
    ...
    def test_query_by_text_obj(self):
        # WIP : GH10846
        name_text = appalchemy.text("select * from iris where name=:name")
    ...
    def test_query_by_select_obj(self):
        ...
        name_select = appalchemy.select([iris]).where(
            iris.c.Name == appalchemy.bindparam("name")
        )
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    ...
    def setup_connect(self):
        try:
            ...
            self.conn.connect()
        except appalchemy.exc.OperationalError:
    ...
    def test_dtype(self):
        ...
        df.to_sql("dtype_test2", self.conn, dtype={"B": appalchemy.TEXT})
        meta = appalchemy.schema.MetaData(bind=self.conn)
        meta.reflect()
        sqltype = meta.tables["dtype_test2"].columns["B"].type
        assert isinstance(sqltype, appalchemy.TEXT)
        msg = "The type of B is not a SQLAlchemy type"
        with pytest.raises(ValueError, match=msg):
            df.to_sql("error", self.conn, dtype={"B": str})

        # GH9083
        df.to_sql("dtype_test3", self.conn, dtype={"B": appalchemy.String(10)})
        meta.reflect()
        sqltype = meta.tables["dtype_test3"].columns["B"].type
        assert isinstance(sqltype, appalchemy.String)
        assert sqltype.length == 10

        # single dtype
        df.to_sql("single_dtype_test", self.conn, dtype=appalchemy.TEXT)
        meta = appalchemy.schema.MetaData(bind=self.conn)
        meta.reflect()
        sqltypea = meta.tables["single_dtype_test"].columns["A"].type
        sqltypeb = meta.tables["single_dtype_test"].columns["B"].type
        assert isinstance(sqltypea, appalchemy.TEXT)
        assert isinstance(sqltypeb, appalchemy.TEXT)
    ...
    def test_double_precision(self):
        ...
        df.to_sql(
            ...
            index=False,
            if_exists="replace",
            dtype={"f64_as_f32": appalchemy.Float(precision=23)},
        )
    ...
    def test_temporary_table(self):
        ...
        class Temporary(Base):
            ...
            id = appalchemy.Column(appalchemy.Integer, primary_key=True)
            spam = appalchemy.Column(appalchemy.Unicode(30), nullable=False)
        ...
        with session.transaction:
            ...
            session.flush()
            df = sql.read_sql_query(sql=appalchemy.select([Temporary.spam]), con=conn)
...
class _TestMySQLAlchemy:
    ...
    @classmethod
    def connect(cls):
        return appalchemy.create_engine(
...
class _TestPostgreSQLAlchemy:
    ...
    def test_schema_support(self):
        ...
        if isinstance(self.conn, appalchemy.engine.Engine):
            engine2 = self.connect()
            meta = appalchemy.MetaData(engine2, schema="other")
